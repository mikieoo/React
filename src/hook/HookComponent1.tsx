...
// - useReducer : 상태 로직을 관리하는 함수, 상태 업데이트 로직을 컴포넌트 외부로 옮길 수 있게히여 코드 관리를 수월하게 도와줌
// - useCallback : 메모리 내에서 함수를 재사용할 수 있도록 하는 함수, 렌더링없이 특정 함수를 실행할 수 있게 함
// - useMemo : 메모리 내에 계산된 값(함수 결괴)을 저장하여 함수 재실행 없이 값을 사용할 수 있도록 하는 함수
...
export default function HookComponent1() {
    ...
    // useEffect : 
    // - 컴포넌트의 생명주기에 따라 특정 함수를 호출하도록 하는 함수
    // - mount : 컴포넌트가 처음 화면에 등록되었을 떄
    // - update : 컴포넌트 상태가 변경되어 리렌더링 되었을 때 
    // - unmount : 컴포넌트가 화면에서 제거될 때 
    // - useEffect(호출할 콜백 함수, 스코프할 상태 배열);
    // - 개발 실행환경에서는 mount - unmount가 한번 실행되고난 후에 다시 mount가 됨
    ...
    // 스코프할 상태 배열을 지정하지 않은 경우 모든 상태갸 바뀔때마다 실행됨
    // useEffect(() => {
...
function SubComponent() {
    ...
    // 전달한 콜백 함수의 return으로 컴포넌트가 unmount 될때 실행할 함수를 반환
